...
smooth = 0 #为1则平滑某一时刻的written_bytes
timestep = 4
regionstep = 2
    ...
    with open(dirname + files[k], 'r') as f:
        ...
        if smooth == 1:
            ...
            for i in range(1, len(data) - 1):
                newdata[i][2] = (data[i - 1][2] + data[i][2] + data[i + 1][2]) / 3
            data = newdata

        written_sum = 0
        for i in range(len(data)):
            written_sum += data[i][2]
            if (i+1)% regionstep == 0:
                if written_sum < 150000000:
                    X.append(k)
