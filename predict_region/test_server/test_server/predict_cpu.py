...
def get_cpu_input(output_size, url):
    sum = 0

    '''
    cpu_list = []
    with open('cpu_data/1.txt', 'r') as f:
...
def train_lstm(input_size,output_size,lr,rnn_unit,weights,biases,batch_size,time_step,kp,url,
               predict_step,save_model_path,save_model_name):
        ...
        while True:#每个预测就是一次循环，每隔一段时间就训练一次，存储模型
            ...
            if label >= time_step:
                ...
                for i in range(output_size):
                    ...
                    #计算期望副本数
                    #print("预测的总cpu使用率：", predict[i])
                    pre_replicas = ceil(predict[i] / 4)
                    if pre_replicas > 6:
                        pre_replicas = 6
                    if pre_replicas < 4:
                        pre_replicas = 4
                    #print("预测的replicas：", pre_replicas)
                    tempVar.data['replicas'] =pre_replicas
                    print("label_cpu:%d, time:%s, predict_cpu_usage:%f, predict_replicas:%d"
                          %(label,time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) ,predict[i],pre_replicas))
            if label < time_step:
                print('label_cpu:', label)
...
def start_predict_cpu():
    ...
    url = 'http://10.233.12.211:2379/pd/api/v1/regions'
    save_model_path = './save/predict_cpu_4-7/'  # checkpoint存在的目录
    ...
    train_lstm(input_size,output_size,lr,rnn_unit,weights,biases,batch_size,
               time_step,kp,url,predict_step,save_model_path,save_model_name)
