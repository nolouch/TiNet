...
def get_cpu_input(output_size, url, interval):
    ...
    if interval == 0:
        ...
        cpudata = fetch_tidb_cpu_usage(prome_addr, start, end, step)
        for ins in cpudata:
            sum += float(ins['values'][0][1])
        history_data[0:-1] = history_data[1:]  # 丢掉最早的时刻
        history_data[-1][0] = sum
...
def train_lstm(input_size,output_size,lr,rnn_unit,weights,biases,batch_size,time_step,kp,url,
               predict_step,save_model_path,save_model_name,init_tidb_replicas,yaml_path):
    ...
    with tf.Session() as sess:
        ...
        current_replicas = init_tidb_replicas
        label = 0  # 当前的序号
        ...
        while True:#每个预测就是一次循环，每隔一段时间就训练一次，存储模型
            ...
            for i in range(output_size):
                ...
                tempVar.data['tidb_replicas'] = pre_replicas
                print("label_cpu:%d, time:%s, predict_step:%d, predict_tidb_cpu_usage:%f, predict_tidb_replicas:%d"
                ...
                if (label - last_scale_time >= scaleIntervalmins) and (pre_replicas != current_replicas) :
                    ...
                    with open(yaml_path, "w") as yaml_file:
                        generate_dict['spec']['tidb']['replicas'] = pre_replicas
                    ...
                    print("output: ", output)
                    if exitcode == 0:
                        print("Execute scaling command, tidb_replicas:%d -> %d"%(current_replicas,pre_replicas))
...
def start_predict_cpu():
    ...
    predict_step = 3  #预测predict_step分钟后的负载
    kp = 1  # dropout保留节点的比例
    tempVar.data = globalvar.get_demo_value()
    url = 'http://10.233.22.61:2379/pd/api/v1/regions'
    save_model_path = './save/predict_tidb_cpu/'  # checkpoint存在的目录
    ...
    cpurequest, init_tidb_replicas = yaml_to_dict(yaml_path)#读配置的limits，和初始的tidb的replicas
    ...
    train_lstm(input_size,output_size,lr,rnn_unit,weights,biases,batch_size,
               time_step,kp,url,predict_step,save_model_path,save_model_name,init_tidb_replicas,yaml_path)
